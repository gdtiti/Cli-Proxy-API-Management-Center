...
export function SystemPage() {
  ...
  const authConnectionStatus = useAuthStore((state) => state.connectionStatus);
  const authServerVersion = useAuthStore((state) => state.serverVersion);
  const authServerBuildDate = useAuthStore((state) => state.serverBuildDate);
  ...
  const canEditRequestLog = authConnectionStatus === 'connected' && Boolean(config);

  const appVersion = __APP_VERSION__ || t('system_info.version_unknown');
  const apiVersion = authServerVersion || t('system_info.version_unknown');
  const buildTime = authServerBuildDate
    ? new Date(authServerBuildDate).toLocaleString(i18n.language)
  ...
  const fetchModels = async ({ forceRefresh = false }: { forceRefresh?: boolean } = {}) => {
    if (authConnectionStatus !== 'connected') {
      setModelStatus({
        type: 'warning',
    ...
    try {
      ...
      const list = await fetchModelsFromStore(authApiBase, primaryKey, forceRefresh);
  ...
  }, [authApiBase, authConnectionStatus]);
        ...
        <Card className={styles.aboutCard}>
          ...
          <div className={styles.aboutInfoGrid}>
            ...
            <div className={styles.infoTile}>
              ...
              <div className={styles.tileValue}>{t(`common.${authConnectionStatus}_status`)}</div>
